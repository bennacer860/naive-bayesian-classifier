...
class NaiveBayes
  ...
  attr_accessor :words,:categories_documents,:categories_words,:total_words,:total_documents
  ...
  #save state and make it persistent
  def save_state
  #serialize variables
    File.open('_words', 'w+') do |f|
      Marshal.dump(@words, f)
    end

    File.open('_total_words', 'w+') do |f|
      Marshal.dump(@total_words, f)
    end

    File.open('_total_documents', 'w+') do |f|
      Marshal.dump(@total_documents, f)
    end

    File.open('_categories_documents', 'w+') do |f|
      Marshal.dump(@categories_documents, f)
    end

    File.open('_categories_words', 'w+') do |f|
      Marshal.dump(@categories_words, f)
    end
  ...
  def load_state
   ...
   #serialize variables
   if(File.exist?('_words') && File.exist?('_total_words') && File.exist?('_total_documents') && File.exist?('_categories_documents') && File.exist?('_categories_words') )
    
      File.open('_words') do |f|
        @words=Marshal.load(f)

      end

      File.open('_total_words') do |f|
        @total_words =Marshal.load(f)
      end

      File.open('_total_documents') do |f|
        @total_documents=Marshal.load(f)
      end

      File.open('_categories_documents') do |f|
        @categories_documents= Marshal.load(f)
      end

      File.open('_categories_words') do |f|
        @categories_words=Marshal.load(f)
      end
    else
  ...
  def best_type_score(document)
    ...
    pretty =pretty.sort {|a,b| a[1]<=>b[1]}
    best=pretty.pop
    return best
  end

 def type_scores(document)
    probs = probabilities(document).sort {|a,b| a[1]<=>b[1]}
    totals = 0
    pretty = Hash.new
    probs.each { |prob| totals += prob[1]}
...
# puts "p(#{word[0]})= #{word_probability(category, word[0])} "
       doc_prob *= word_probability(category, word[0]) unless ((doc_prob * word_probability(category, word[0]) ) < 10**(-300) )
    end
    return doc_prob
  end
...
# puts "prob(c|d)= category_probability #{category_probability(category)} * doc_probability #{doc_probability(category, document)}"
# puts "-"*50
# puts "----------------------- #{category} -------------------------"
